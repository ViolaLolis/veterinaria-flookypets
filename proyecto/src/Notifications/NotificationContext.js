...
export const NotificationProvider = ({ children }) => {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            // Utiliza la ruta corregida en el backend
        ...
        if (isDbNotification) {
            try {
                ...
                const response = await authFetch(`/api/notifications/${notificationId}/read`, {
                    method: 'PUT',
                    body: JSON.stringify({ leida: true }), // Asegúrate de enviar el cuerpo con el estado
                });
                if (response.success) {
                    setNotifications(prevNotifications =>
    ...
    const removeNotification = useCallback(async (notificationId, isDbNotification = true) => {
        if (isDbNotification) {
            try {
                ...
                addNotification({ message: `Error de red al eliminar notificación: ${error.message}`, type: 'error', ephemeral: true });
            }
